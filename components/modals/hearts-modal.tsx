...
export const HeartsModal = () => {
  ...
  const onClick = (msg: string) => {
    ...
    const clearMsg = msg.replace(" ", "%20")

    if (mobileCheck()) {
      window.location.href = `https://wa.me/${data.whatsapp}?text=${clearMsg}`;
    } else {
      window.location.href = `https://web.whatsapp.com/send/?phone=${data.whatsapp}?text=${clearMsg}`;
        ...
        <DialogFooter className="mb-4">
          <div className="flex w-full flex-col gap-y-4">
            <Button
              ...
              className="w-full"
              size="lg"
              onClick={() => onClick("Dame un abrazo 💘")}
            >
